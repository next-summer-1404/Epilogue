...
'use client'
import { bookingApi } from "@/core/services/api/booking.api"
...
const HouseHeader = () => {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      setLoading(true)
      try {
        // 👇 اینجا ID رزرو رو تستی وارد کن (عدد یا UUID)
        const data = await bookingApi("12345")
        setBookingData(data)
      } catch (error) {
  ...
  return (
    <div className="w-full flex flex-col gap-6 px-4 lg:px-16">
      <h2 className="text-xl font-semibold text-[#8CFF45]">جزئیات رزرو</h2>

      {loading ? (
        <p className="text-gray-400">در حال بارگذاری...</p>
      ) : bookingData ? (
        <pre className="bg-[#1e1e1e] text-white p-4 rounded-xl overflow-auto">
